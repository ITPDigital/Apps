...
export default class MarqueeHorizontal extends Component {
    ...
    textOnLayout = (e) => {
        ...
        this.setState({
            textWidth: width + ((textList ? textList.length - 1: 0) * separator),
        })
    }
        ...
        for (let i = 0; i < list.length; i++) {
            ...
            itemView.push(
                <TouchableOpacity key={'' + i} activeOpacity={0.9} onPress={() => {
                    ...
                    <View style={{ flexDirection: 'row', marginRight: i < list.length - 1 ? separator : 0,justifyContent:"center"}}>
                        <Text style={{
                            ...styles.textStyle,
                            ...textStyle,
                            textAlign:"center",
