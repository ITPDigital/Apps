const ORANGE = "rgb(255, 116, 78)";
// const RED = "rgb(255, 71, 108)";
const WHITE = "rgb(255, 255, 255)"; // text, bg, 1
const TRANSPARENT = "rgba(0, 0, 0, 0.3)";
const BLACK = "rgb(0,0,0)"; // text , bg, 3
const GREY = "rgba(216,216,216,0.2)"; // line
const GREY100 = "rgb(216,216,216)"; // line
const BLACK2 = "rgb(18,29,55)"; // text , curosel-dot
const BLACK3 = "rgb(29,29,29)"; // background
const GREY2 = "rgb(112,113,114)"; // text
const BLACK4 = "rgb(49,49,50)"; // article heading
const PINK = "rgb(255,51,102)"; // text , line, border
const GREY3 = "rgb(133,136,139)"; // text
const GREY4 = "rgb(49,49,50)"; // text
const GREY5 = "rgb(140,142,145)"; // curosel-dot
const GREY6 = "rgb(136,149,169)"; // text
const WHITE_YELLOW = "rgb(248,242,229)"; // bg, 2
const GREY7 = "rgb(90,90,92)"; // bg, 4
const GREY8 = "rgb(51,51,51)"; // bg, 5
const DARK_BLUE = "rgb(18,29,55)"; // bg
const GREY9 = "rgb(240,242,244)"; // line
const GREY10 = "rgb(110,110,112)"; // border color
const RED = "rgb(255,0,0)"; // text
const LIGHTBLACK = "#1d1d1d";
const yellow = "#feee00";
const GREY11 = "rgb(219, 228, 242)";
const WHITE_OPACITY = "rgba(255,255,255,.5)";
const BLACK5 = "rgb(27,27,27)"; // text , bg
const GRAY12 = "rgb(229,229,229)";
const BLACK6 = "rgb(18,29,55)";
const GRAY13 = "rgb(219,228,242)";
const GRAY14 = "rgb(108,108,108)";
const GRAY15 = "rgb(189,189,189)";
const PINK2 = "rgb(234,0,139)";
const BLACK9 = "rgb(18,29,55)";
const RED1 = "rgb(255,51,102)"; // leadText
const GREYLIGHT = "#85888B";

const Colors = {
	bgPrimaryDark: BLACK,
	// bgPrimaryLight: WHITE,
	bgPrimaryLight: BLACK,   
	bgSecondaryDark: GREY8,
	bgSecondaryLight: WHITE_YELLOW,
	bgTertiaryDark: GREY7,
	bgTertiaryLight: GREY5,
	bgTransparent: TRANSPARENT,
	//bgPrimaryVarient: PINK,
	bgPrimaryVarient: BLACK,
	bgSecondaryVarient: DARK_BLUE,
	bgPrimaryBlack: BLACK3,
	bgDrawer: BLACK5,
	bgPink: PINK2,
	bgOrange: ORANGE,

	bodyPrimaryDark: BLACK,
	bodyPrimaryLight: WHITE,
	bodySecondaryDark: BLACK2,
	// bodySecondaryLight: GREY3,
	bodySecondaryGrey: GREY3,

	bodySecondaryLight: BLACK,
	// bodyTertiaryDark: GREY4,
	// bodyTertiaryLight: GREY6,
	bodyTime: GREYLIGHT,
	bodyTertiaryDark: BLACK,
	bodyTertiaryLight: BLACK,
	bodyPrimaryVarient: RED,
	bodySecondaryVarient: RED,
	bodyTertiaryVarient: GREY2,

	linePrimary: GREY,
	lineSecondary: GREY9,
	brandPrimary: yellow,
	// linePrimaryOpacity: WHITE_OPACITY,
	linePrimaryOpacity: BLACK,
	// BACKGROUND
	bgLightBlack: LIGHTBLACK,
	textHeading: BLACK4,

	// border color
	borderlight: GREY10,
	borderSeparator: GREY11,

	// searchHeader
	searchHeader: GRAY12,
	searchIcon: BLACK6,
	borderLine: GRAY13,
	linePrimaryFull: GREY100,

	// videodetail
	textColor: GRAY14,
	commentText: GRAY15,
	itemHeader: BLACK9,
	leadText: RED1,
};

export default Colors;
