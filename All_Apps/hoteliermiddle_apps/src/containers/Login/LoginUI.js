...
import { View, StyleSheet, Text, ActivityIndicator, Image, I18nManager } from "react-native";
...
export default class LoginUI extends PureComponent<Props> {
	...
	handleForm = () => {
		const { handleLogin, handleForgotPassword, showLoader } = this.props;
		...
		return (    
			<View style={styles.formStyle}>
				<TextInput
					...
					//value="sagarchavan491@gmail.com"
	...
	renderButton = () => {
		...
		const { email, password } = this.state;
		console.log("onesignal open ", "OneSignal.configure");
		OneSignal.configure();
			...
			<Button
				...
				onPress={
					!showLoader
						? () => {
								OneSignal.configure();
								handleLogin(email, password, this.clearText);
						  }
						: null
				}
