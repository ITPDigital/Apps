...
const renderBackbtn = (onBack: Function, onGrid: Function, contentColor) => {
	return onBack ? (
		<TouchableOpacity onPress={onBack} style={styles.buttonContainer}>
			...
			<Icon name={Images.back} size={14} color={contentColor || Colors.bgPrimaryDark} />
		}
		</TouchableOpacity>
	) : (
...
const renderTitle = (title: string, isLogo: boolean, logoUrl: string) => {
	...
	if (logoUrl) {
		...
		content = (
			...
			// 	svgXmlData={logo}
			// />
			<Image source={Images.ABlogo} resizeMode="contain" style={styles.abLogo} />
		);
	}
	return <View style={styles.titleContainer}>{content}</View>;
};

export default function TransparentHeader(props: Props) {
	const {
		style,
		onAction,
		onBack,
...
const styles = StyleSheet.create({
	...
	width: 150,
	height: 15,
	marginLeft: ScalePerctFullWidth(10),
});
