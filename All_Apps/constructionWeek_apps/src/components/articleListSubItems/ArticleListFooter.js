...
export default function ArticleListFooter(props: Props) {
  ...
  console.log('item in list footer', JSON.stringify(tabContainerStyle));
  return (
    <View style={[styles.container, tabContainerStyle || null]}>
      <View
        style={{
          width: bookmarkRequired && isBookMarkNeeded ?  ScalePerctFullWidth(45) :  ScalePerctFullWidth(40),
      ...
      >
        <SvgUri width={14} height={14} svgXmlData={share} />
      </TouchableOpacity>
      {bookmarkRequired && isBookMarkNeeded && (
        <TouchableOpacity
          onPress={() => onBookMarkToggle()}
          style={[ { alignSelf: 'flex-end'},iconStyle]}
        >
          {isBookMarked ? (
            // <Icon
            ...
            // />
            <SvgUri width={14} height={14} svgXmlData={bookmark} />
          ) : (
            // <Image
            // 	resizeMode="contain"
            ...
            // 	// }}
            // />
            <SvgUri width={14} height={14} svgXmlData={unBookMark} />
          )}
        </TouchableOpacity>
      )}
    </View>
...
const styles = StyleSheet.create({
  ...
  hours: {
    //paddingTop: 4,
    fontSize: 9, //Metrics.MEDIUM_TEXT_SIZE,
    letterSpacing: 0.3,
    textAlign: 'left',
    alignSelf: 'stretch',
    flexWrap: 'wrap',
